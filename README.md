...
The backend sends two emails automatically whenever someone books a table:
a confirmation to the customer, and a notification to you (the restaurant).
It also emails you when someone submits the Contact form. Here's how to
turn that on using a free Gmail account.

**1. Turn on 2-Step Verification** (required before Google will let you create an app password)
- Go to [myaccount.google.com/security](https://myaccount.google.com/security) and enable it if it isn't already.

**2. Create an App Password**
- Go to [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords)
- Under "App name," enter `Urban Spice` → **Create**
- Google shows a 16-character password (e.g. `abcd efgh ijkl mnop`) — copy it. This is **not** your normal Gmail password, and Gmail will reject your real password if you use it here instead.
...
EMAIL_PORT=587
EMAIL_SECURE=false
EMAIL_USER=yourrestaurant@gmail.com
EMAIL_PASSWORD=abcdefghijklmnop
EMAIL_FROM_NAME=Urban Spice
EMAIL_FROM_ADDRESS=yourrestaurant@gmail.com
RESTAURANT_ADMIN_EMAIL=yourrestaurant@gmail.com
```

| Variable | What it actually does |
|---|---|
| `EMAIL_USER` / `EMAIL_PASSWORD` | The Gmail account that **sends** the email (uses the app password from step 2) |
| `RESTAURANT_ADMIN_EMAIL` | The inbox that **receives** booking/contact notifications — this is the one that matters most to you as the owner |

`EMAIL_USER` and `RESTAURANT_ADMIN_EMAIL` can be the exact same address — that's the simplest setup for a single-owner restaurant. Gmail just ends up sending the notification to itself, which works completely fine.

**4. Restart the backend and test it**

```bash
npm run dev
```

Submit a real reservation through the site. You should get a notification at `RESTAURANT_ADMIN_EMAIL`, and the email address used in the booking should get a confirmation.

**Using a different email provider instead of Gmail?** Any SMTP provider works (Outlook, a custom domain email, SendGrid, etc.) — just set `EMAIL_HOST`, `EMAIL_PORT`, and `EMAIL_SECURE` to match that provider's SMTP settings, and use its credentials for `EMAIL_USER`/`EMAIL_PASSWORD`.
